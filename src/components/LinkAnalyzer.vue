...
const mockAnalytics = ref({
  ...
  topDevices: [
    { type: 'Desktop', percentage: 64, color: 'bg-zinc-100' },
    { type: 'Mobile', percentage: 31, color: 'bg-blue-500' },
    { type: 'Tablet', percentage: 5, color: 'bg-zinc-600' }
  ],
  topReferrers: [
  ...
  <section class="py-16 sm:py-24 border-t border-[var(--border-color)] bg-[var(--bg-secondary)]/50 transition-colors duration-200" id="analyzer">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
      ...
      <div class="text-center max-w-2xl mx-auto space-y-3">
        <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-950/60 border border-blue-800 text-blue-400 text-xs font-semibold">
      ...
      <div class="max-w-xl mx-auto">
        <form @submit.prevent="handleAnalyzeSearch" class="relative flex items-center">
          ...
          <input
            ...
            class="w-full pl-11 pr-28 py-3 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl text-sm text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-zinc-100 transition shadow-sm"
          />
          <button
            type="submit"
            :disabled="isAnalyzing"
            class="absolute right-1.5 px-4 py-1.5 bg-zinc-100 text-zinc-900 rounded-lg text-xs font-semibold hover:bg-zinc-200 transition cursor-pointer shadow-xs"
          >
      ...
      <!-- Dashboard Grid Box -->
      <div class="bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-2xl p-6 sm:p-8 shadow-none space-y-8">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[var(--border-color)] pb-6">
          <div>
            <div class="flex items-center gap-2">
              <span class="text-xs font-mono font-semibold text-blue-400 uppercase tracking-wider">Active Stream</span>
        ...
        <div class="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div class="p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] space-y-1">
            <div class="flex items-center justify-between text-xs text-[var(--text-secondary)]">
              ...
              <MousePointerClick class="w-4 h-4 text-blue-400" />
            </div>
          ...
          <div class="p-4 sm:p-6 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] overflow-x-auto">
            <div class="h-44 min-w-[300px] flex items-end justify-between gap-2 sm:gap-4 pt-4">
              ...
              >
                ...
                <div class="w-full bg-zinc-800 rounded-t-sm h-36 flex items-end overflow-hidden">
                  <div
                    class="w-full bg-zinc-100 group-hover:bg-blue-500 transition-all rounded-t-sm"
            ...
            <div class="space-y-3">
              <div v-for="country in mockAnalytics.topCountries" :key="country.code" class="space-y-1">
                ...
                <div class="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                  <div class="h-full bg-zinc-100 rounded-full" :style="{ width: `${country.percentage}%` }"></div>
                </div>
              </div>
            </div>
          </div>
