...
const handleShorten = async () => {
  ...
  try {
    let testUrl = longUrl.value
    if (!/^https?:\/\//i.test(testUrl)) {
      testUrl = 'https://' + testUrl
      longUrl.value = testUrl
    }
    new URL(testUrl)
  } catch {
    errorMsg.value = 'Please enter a valid URL format'
    return
  }
  ...
  // Simulate shortener delay for realistic feel
  setTimeout(() => {
    const alias = customAlias.value.trim()
      ? customAlias.value.trim().replace(/[^a-zA-Z0-9_-]/g, '')
      : Math.random().toString(36).substring(2, 8)

    shortenedLink.value = `https://linkly.sh/${alias}`
...
const downloadQR = (format: 'png' | 'svg') => {
  ...
  if (format === 'png') {
    const image = qrCanvasRef.value.toDataURL('image/png')
    const link = document.createElement('a')
    link.href = image
    link.download = `linkly-qr-${Date.now()}.png`
    link.click()
  } else {
    // Basic SVG fallback download
    const svgData = `<svg xmlns="http://www.w3.org/2000/svg" width="${qrSize.value}" height="${qrSize.value}">
      <foreignObject width="100%" height="100%">
        <img xmlns="http://www.w3.org/1999/xhtml" src="${qrCanvasRef.value.toDataURL('image/png')}" width="${qrSize.value}" height="${qrSize.value}" />
      </foreignObject>
    </svg>`
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `linkly-qr-${Date.now()}.svg`
    link.click()
  }
}
</script>
...
<template>
  <div class="w-full max-w-4xl mx-auto" id="shortener">
    <!-- Main Box Container -->
    <div class="bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-2xl p-4 sm:p-6 lg:p-8 shadow-xl shadow-black/5 dark:shadow-none transition-all duration-300">
          ...
          <div class="relative flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
              ...
              <input
                ...
                class="w-full pl-11 pr-4 py-3.5 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl text-sm font-normal text-[var(--text-primary)] placeholder-[var(--text-secondary)] focus:outline-none focus:ring-2 focus:ring-zinc-900 dark:focus:ring-zinc-100 transition"
                required
              />
            </div>
            ...
            <button
              type="submit"
              :disabled="isLoading"
              class="px-6 py-3.5 bg-zinc-900 dark:bg-zinc-100 hover:bg-zinc-800 dark:hover:bg-zinc-200 text-zinc-50 dark:text-zinc-900 rounded-xl font-medium text-sm flex items-center justify-center gap-2 transition cursor-pointer shadow-md disabled:opacity-50"
            >
              <Sparkles v-if="!isLoading" class="w-4 h-4 text-blue-400 dark:text-blue-600" />
          ...
          <div class="pt-2 flex flex-wrap items-center justify-between gap-4 text-xs text-[var(--text-secondary)]">
            <div class="flex items-center gap-2 w-full sm:w-auto">
              ...
              <div class="flex items-center bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg px-2.5 py-1.5">
                <span class="text-[var(--text-secondary)] select-none">linkly.sh/</span>
                <input
                  v-model="customAlias"
                  type="text"
            ...
            <div class="flex items-center gap-4">
              <label class="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" v-model="autoCopy" class="rounded border-zinc-300 text-zinc-900 focus:ring-zinc-500" />
          ...
          <div class="p-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
              <button
                ...
                class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-zinc-50 dark:text-zinc-900 font-medium text-xs transition cursor-pointer"
              >
      ...
      <div v-else class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div class="flex flex-col items-center justify-center p-6 bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-color)]">
            <div class="p-4 bg-white rounded-xl shadow-inner border border-zinc-200">
          ...
          <div class="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                max="320"
                step="20"
                class="w-full accent-zinc-900 dark:accent-zinc-100 cursor-pointer"
              />
            </div>
            ...
            <div class="pt-2 flex items-center gap-3">
              <button
                ...
                class="flex-1 py-2.5 px-4 bg-zinc-900 dark:bg-zinc-100 text-zinc-50 dark:text-zinc-900 rounded-xl text-xs font-medium flex items-center justify-center gap-2 hover:bg-zinc-800 dark:hover:bg-zinc-200 transition cursor-pointer"
              >
              ...
              <button
                ...
                class="flex-1 py-2.5 px-4 bg-[var(--bg-primary)] border border-[var(--border-color)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] rounded-xl text-xs font-medium flex items-center justify-center gap-2 transition cursor-pointer"
              >
                <Download class="w-4 h-4" />
